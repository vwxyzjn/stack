...
provider "kubernetes" {
  host = aws_eks_cluster.wandb.endpoint

  cluster_ca_certificate = base64decode(aws_eks_cluster.wandb.certificate_authority[0].data)
    ...
    template {
      ...
      spec {
        container {
          ...
          env {
            name  = "MYSQL"
            value = "mysql://wandb:${var.db_password}@${aws_rds_cluster_instance.metadata_store.endpoint}/wandb_local"
          }
